...
class ClickerService : AccessibilityService() {
    ...
    private fun showController() {
        ...
        col.addView(playBtn)
        col.addView(pill("＋") { addTarget("tap") })              // add tap point
        col.addView(pill("⇢", SWIPE_ACCENT) { addTarget("swipe") }) // add swipe
        col.addView(pill("⊕", 0xFFE0C43B.toInt()) { addTarget("hold") }) // add long-press
        ...
        // whole bar draggable from the ✥ handle
        var sx = 0; var sy = 0; var tx = 0f; var ty = 0f
